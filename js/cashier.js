...
function getCashierProfit(){
  const grossProfit = _cashierItems.reduce((sum, item) => {
    const unitPrice = parseFloat(item.price) || 0;
    const unitCost = parseFloat(item.prod?.cost) || 0;
    const qty = parseInt(item.qty, 10) || 0;
    return sum + ((unitPrice - unitCost) * qty);
  }, 0);
  return Math.max(0, grossProfit - _cashierDiscount);
}

function getCashierCost(){
  return _cashierItems.reduce((sum, item) => {
    const unitCost = parseFloat(item.prod?.cost) || 0;
    const qty = parseInt(item.qty, 10) || 0;
...
function calcCashierInstall(){
  const baseTotal = getCashierTotal();
  const extra = parseFloat(document.getElementById('ci-extra')?.value)||0;
...
async function confirmCashierSale(){
  ...
  if(type==='installment'){
    ...
    const months= parseInt(document.getElementById('ci-months').value)||1;
    const installmentBase = total;
    const installmentTotal = installmentBase + extra;
    const installmentCost = getCashierCost();
    const installmentProfit = Math.max(0, installmentTotal - installmentCost);
    const installmentCapitalLoss = Math.max(0, installmentCost - installmentTotal);
    ...
    insts.push({id:genId(), customerName:name, phone, productName:productNames,
      totalPrice:installmentTotal, basePrice:installmentBase, extraProfit:extra,
      downPayment:down, months, monthlyPayment:monthly,
      profit:installmentProfit, cost:installmentCost,
      expectedSalePrice:installmentTotal, expectedProfit:installmentProfit,
      profitLoss:0, capitalLoss:installmentCapitalLoss,
      paid:down, remaining:remain, paidMonths:0,
    ...
    sales.push({id:instSaleId, productName:productNames, type:'installment',
      qty:_cashierItems.reduce((s,i)=>s+i.qty,0),
      totalPaid:down, profit:installmentProfit, cost:installmentCost,
      expectedSalePrice:installmentTotal, expectedProfit:installmentProfit,
      profitLoss:0, capitalLoss:installmentCapitalLoss,
      date:nowISO(),
    ...
    DB.addTransaction({
      ...
      qty:_cashierItems.reduce((s,i)=>s+i.qty,0),
      salePrice:installmentTotal, profit:installmentProfit,
