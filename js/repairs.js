...
function setRepairFilter(f){
  ...
  ['day','week','month'].forEach(x=>{
    document.getElementById('rep-tab-'+x).classList.toggle('active', x===f);
  });
  renderRepairs();
}

function getRepairFrom(){
  const now = new Date();
  if(_repFilter==='day'){ const d=new Date(now); d.setHours(0,0,0,0); return d; }
  if(_repFilter==='week'){ const d=new Date(now); d.setDate(d.getDate()-7); return d; }
...
function getNextSerial(){
  ...
  return reps.length ? Math.max(...reps.map(r=>r.serialNo||0))+1 : 1;
}

function openRepairModal(editId=null){
  _repEditId = editId;
  ['rm-name','rm-phone','rm-device','rm-service'].forEach(id=>{
    const el = document.getElementById(id);
    if(el) el.value='';
  });

  // توليد رقم تسلسلي جديد
  const bc13El = document.getElementById('rm-barcode13');
  if(bc13El) bc13El.value = genRandom13();

  if(editId){
    const reps = DB.get('repairs');
    const r = reps.find(x=>x.id===editId);
    if(!r) return;
    document.getElementById('rep-modal-title').textContent='تعديل عملية التصليح';
    document.getElementById('rm-name').value   = r.customerName||'';
    document.getElementById('rm-phone').value  = r.phone||'';
    document.getElementById('rm-device').value = r.device||'';
    const svcEl = document.getElementById('rm-service');
    if(svcEl) svcEl.value = r.servicePrice||r.total||0;
    if(bc13El) bc13El.value = r.barcode13||genRandom13();
  } else {
    document.getElementById('rep-modal-title').textContent='تسجيل عملية تصليح';
  }
  openModal('rep-modal-ov');
}

function saveRepair(){
  const name   = document.getElementById('rm-name').value.trim();
  const device = document.getElementById('rm-device').value.trim();
  if(!name)  { toast('أدخل اسم الزبون','err'); return; }
  if(!device){ toast('أدخل الجهاز والعطل','err'); return; }

  const svcEl   = document.getElementById('rm-service');
  const service = parseFloat(svcEl ? svcEl.value : 0)||0;
  const bc13El  = document.getElementById('rm-barcode13');
  const barcode13 = bc13El ? bc13El.value||genRandom13() : genRandom13();

  const reps = DB.get('repairs');

  if(_repEditId){
    const idx = reps.findIndex(r=>r.id===_repEditId);
    if(idx>=0){
      reps[idx]={...reps[idx],
        customerName:name,
        ...
        total:service,
        // حفظ القديم للتوافق
        partsCost:0, profitSiraj:0, profitAnis:0,
        barcode13: reps[idx].barcode13||barcode13
      };
    }
    DB.set('repairs',reps);
    ...
    const rec={
      ...
      partsCost:0, profitSiraj:0, profitAnis:0,
      barcode13,
      paid:false, paidDate:null, date:nowISO()
    };
    reps.push(rec);
    DB.set('repairs',reps);
    // مزامنة Firestore
    fsSaveDoc('repairs', rec.id, rec);
    tg(`🔧 <b>عطل جديد</b>\nالزبون: ${name}\nالجهاز: ${device}\nسعر الخدمة: ${fmt(service)}\nالرقم: #${serialNo}\nالتاريخ: ${todayStr()}`);
    toast('✅ تم تسجيل العطل');
  }
  closeModal('rep-modal-ov');
  renderRepairs();
}
...
function openRepairDetail(id){
  ...
  if(!r) return;
  document.getElementById('rd-name').textContent   = r.customerName;
  document.getElementById('rd-serial').textContent = '#'+r.serialNo;
  document.getElementById('rd-device').textContent = r.device;
  document.getElementById('rd-phone').textContent  = r.phone||'—';
  document.getElementById('rd-siraj').textContent  = fmt(r.profitSiraj);
  document.getElementById('rd-anis').textContent   = fmt(r.profitAnis);
  document.getElementById('rd-total').textContent  = fmt(r.total);
  openModal('rep-detail-ov');
}

function openRepairPaid(id){
  const reps = DB.get('repairs');
  _repTarget = reps.find(r=>r.id===id);
  if(!_repTarget) return;
  document.getElementById('rpaid-name').textContent   = _repTarget.customerName;
  document.getElementById('rpaid-device').textContent = _repTarget.device;
  document.getElementById('rpaid-siraj').textContent  = fmt(_repTarget.profitSiraj);
  document.getElementById('rpaid-anis').textContent   = fmt(_repTarget.profitAnis);
  document.getElementById('rpaid-total').textContent  = fmt(_repTarget.total);
  openModal('rep-paid-ov');
}

function confirmRepairPaid(){
  if(!_repTarget) return;
  const reps = DB.get('repairs');
  const idx  = reps.findIndex(r=>r.id===_repTarget.id);
  if(idx<0) return;

  reps[idx].paid     = true;
  reps[idx].paidDate = nowISO();
  DB.set('repairs', reps);
  // مزامنة Firestore
  fsSaveDoc('repairs', reps[idx].id, reps[idx]);

  // التصليح لا يُسجَّل في الأرباح — منفصل عن دخل المحل
  const service = _repTarget.servicePrice||_repTarget.total||0;
  tg(`✅ <b>دفع تصليح</b>\nالزبون: ${_repTarget.customerName}\nالجهاز: ${_repTarget.device}\nسعر الخدمة: ${fmt(service)}\nالتاريخ: ${todayStr()}`);

  closeModal('rep-paid-ov');
...
function deleteRepair(id){
  const reps = DB.get('repairs');
  const r = reps.find(x=>x.id===id);
  showConfirm('حذف العملية',`هل تريد حذف عملية "${r?.customerName||'هذا الزبون'}"؟`, async ()=>{
    DB.deleteOne('repairs', id);
...
function printRepairNewLabel(){
  if(!_repTarget) return;
  waitForJsBarcode(()=>{
    const r = _repTarget;
    const d = new Date(r.date);
    const dateStr = `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
    const service = r.servicePrice||r.total||0;

    // باركود 13 عشوائي — إذا لم يكن موجوداً نولّده ونحفظه
    let barcode = r.barcode13;
    if(!barcode){
      barcode = genRandom13();
      const reps = DB.get('repairs');
      const idx = reps.findIndex(x=>x.id===r.id);
      if(idx>=0){ reps[idx].barcode13 = barcode; DB.set('repairs', reps); }
    }

    const tempSvg = document.createElementNS('http://www.w3.org/2000/svg','svg');
    tempSvg.id='_temp_rep_bc';
    tempSvg.style.cssText='position:absolute;top:-9999px';
    document.body.appendChild(tempSvg);
    try{
      JsBarcode('#_temp_rep_bc', barcode, {
    ...
    const paymentStatus = r.paid ? 'تم الدفع' : 'لم يتم الدفع';
    const html = `<div style="width:204mm;height:288mm;display:flex;justify-content:center;align-items:flex-start;box-sizing:border-box;margin:3mm auto 0;overflow:hidden">
      <div style="direction:rtl;font-family:Arial,sans-serif;width:71mm;padding:2mm 2.5mm 0 1.5mm;box-sizing:border-box;text-align:center">
        ...
        <!-- ???????? -->
        <div style="font-size:18px;font-weight:900;margin-bottom:3mm;color:#1a2a5e;text-align:center">${r.customerName}</div>
        <div style="font-size:15px;font-weight:700;margin-bottom:3mm;color:#333;text-align:center">${r.device}</div>
        <div style="font-size:15px;font-weight:700;margin-bottom:2.5mm;text-align:center"><b>المبلغ:</b> ${fmt(service)}</div>
        <div style="font-size:13px;font-weight:600;margin-bottom:2.5mm;color:#555;text-align:center"><b>التاريخ:</b> ${dateStr}</div>
        <div style="font-size:13px;font-weight:600;margin-bottom:4mm;text-align:center"><b>الحالة:</b> ${paymentStatus}</div>
        ...
        <!-- تنبيه + كلمة تصليح -->
        <div style="border-top:2.5px solid #1a2a5e;margin-top:3mm;padding-top:2.5mm">
          <div style="font-size:11px;font-weight:600;color:#555;margin-bottom:2mm;line-height:1.5;text-align:center">المحل غير مسؤول عن الأجهزة<br>التي تتجاوز مدتها 15 يوم</div>
          <div style="border-top:2px dashed #1a2a5e;padding-top:2.5mm">
            <div style="font-size:20px;font-weight:900;color:#1a2a5e;text-align:center">تصليح</div>
          </div>
        </div>
      ...
      doPrint(html.replace('color:#555', 'color:#000'), '210mm', '297mm');
    }catch(e){ tempSvg.remove(); toast('❌ خطأ في الباركود','err'); }
  });
}


function printRepairInfo(){
  if(!_repTarget) return;
  closeModal('rep-print-ov');
  const r = _repTarget;
  const d = new Date(r.date);
  const dateStr = `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
  const html = `<div style="direction:rtl;font-family:Arial,sans-serif;padding:11mm 2.5mm 0;margin:0 auto;text-align:right;width:65mm">
    ...
    <div style="font-size:16px;font-weight:700;margin:6px 0">${r.customerName}</div>
    <div style="font-size:14px;margin:4px 0;color:#333">${r.device}</div>
    <div style="font-size:15px;margin:4px 0">سعر: <b>${fmt(r.total)}</b></div>
    <div style="font-size:13px;margin:4px 0">التاريخ: ${dateStr}</div>
    <div style="font-size:13px;margin:4px 0">الرقم التسلسلي: <b>#${r.serialNo}</b></div>
    <div style="margin-top:8px;padding-top:6px;border-top:1px dashed #999;font-size:14px;font-weight:700">وصل تصليح</div>
...
function printRepairBarcode(){
  if(!_repTarget) return;
  waitForJsBarcode(() => {
  closeModal('rep-print-ov');
  const r = _repTarget;
  const code = 'SJR'+String(r.serialNo).padStart(4,'0');
  const d = new Date(r.date);
  const dateStr = `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`;
  const tempSvg = document.createElementNS('http://www.w3.org/2000/svg','svg');
  tempSvg.id='_temp_rbc';
  tempSvg.style.cssText='position:absolute;top:-9999px';
  document.body.appendChild(tempSvg);
  try{
    JsBarcode('#_temp_rbc',code,{format:'CODE128',width:1.5,height:28,displayValue:true,fontSize:9,margin:2});
    ...
    const html=`<div style="direction:rtl;font-family:Arial,sans-serif;padding:4mm 2.5mm 0;margin:0 auto;text-align:right;width:65mm">
      ...
      <div style="font-size:12.5px;font-weight:600;margin:2px 0">${r.device}</div>
      <div style="display:flex;justify-content:center">${svgHtml}</div>
      <div style="font-size:11.5px;font-weight:600">المبلغ: ${fmt(r.total)} | ${dateStr}</div>
      <div style="font-size:11.5px;font-weight:600">رقم: #${r.serialNo}</div>
    </div>`;
    doPrint(html,'6.5cm','9cm');
